...
function StatCard({ label, value, icon, delay, sub }: {
  ...
  return (
    <div className="glass group relative p-6">
      <div className="relative flex items-start justify-between">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-2">{label}</p>
          <p className="text-4xl font-bold text-[var(--text-primary)] tabular-nums tracking-tight">{count}</p>
          <p className="text-[11px] text-[var(--text-muted)] mt-2 flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--accent)]" style={{ boxShadow: '0 0 6px var(--accent-glow)' }} />
            {sub}
          </p>
        </div>
        <div className="flex flex-col items-center gap-2">
          ...
          <svg width={48} height={48} style={{ transform: 'rotate(-90deg)' }}>
            ...
            <circle cx={24} cy={24} r={r} fill="none" stroke="var(--accent)" strokeWidth={3}
              ...
              style={{ transition: 'stroke-dasharray 1.4s cubic-bezier(0.16,1,0.3,1)', filter: 'drop-shadow(0 0 3px rgba(52,211,153,0.3))' }} />
          </svg>
        </div>
      </div>
    </div>
...
function ActionCard({ href, icon, label, description, badgeText }: {
  ...
  return (
    <Link href={href} className="group flex items-center gap-3 p-4 rounded-xl border border-[var(--glass-border)] bg-[rgba(255,255,255,0.02)] transition-all duration-300 hover:bg-[rgba(255,255,255,0.05)] hover:border-[var(--glass-border-hover)] hover:-translate-y-0.5">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {badgeText && (
            <span className="text-[10px] font-medium px-2 py-0.5 rounded-md bg-[rgba(255,255,255,0.06)] text-[var(--text-secondary)] border border-[var(--glass-border)]">{badgeText}</span>
          )}
        </div>
        <p className="text-[11px] text-[var(--text-muted)] mt-0.5 truncate">{description}</p>
      </div>
      <svg className="w-4 h-4 text-[var(--text-muted)] group-hover:text-[var(--accent)] transition-all duration-200 group-hover:translate-x-0.5 flex-shrink-0"
...
function DeptRow({ name, count, max }: { name: string; count: number; max: number }) {
  ...
  return (
    <div className="flex items-center gap-3 group">
      <div className="w-2 h-2 rounded-full bg-[var(--accent)] flex-shrink-0" style={{ boxShadow: '0 0 5px var(--accent-glow)' }} />
      <span className="text-[13px] text-[var(--text-primary)] font-medium w-36 truncate">{name}</span>
      <div className="flex-1 h-2 rounded-full bg-[rgba(255,255,255,0.06)] overflow-hidden">
        <div className="h-2 rounded-full bg-[var(--accent)] transition-all duration-1000"
          style={{ width: `${pct}%`, boxShadow: '0 0 8px rgba(52,211,153,0.3)' }} />
      </div>
      <span className="text-[11px] font-medium text-[var(--text-muted)] w-14 text-right tabular-nums">{count} {count === 1 ? 'emp' : 'emps'}</span>
    </div>
  );
}
...
export default function AdminDashboard() {
  ...
  const timeStr = mounted && now ? now.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true }) : '--:--';
  const dateStr = mounted && now ? now.toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : 'Loading...';
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="page-hero relative overflow-hidden">
        ...
        />
        <div className="absolute bottom-0 left-0 w-52 h-52 rounded-full blur-[80px] opacity-15 pointer-events-none"
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl lg:text-4xl font-bold tracking-tight mb-1.5">
              <span className="text-gradient">{data.adminUser.name.split(' ')[0]}</span>
          ...
          <div className="flex flex-col items-start lg:items-end gap-2">
            <div className="text-right">
              <div className="text-3xl font-bold text-[var(--text-primary)] tabular-nums tracking-tight">
                {timeStr.split(' ')[0]}
                <span className="text-[13px] text-[var(--text-muted)] ml-1.5">{timeStr.split(' ')[1]}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-2">
        ...
        <div className="glass p-6">
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="ml-auto px-2.5 py-1 rounded-lg bg-[rgba(255,255,255,0.04)] text-[var(--text-muted)] text-[11px] font-medium border border-[var(--glass-border)]">{departmentStats.length} depts</span>
          </div>
          {departmentStats.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <p className="text-[var(--text-muted)] font-medium text-[13px]">No departments populated yet</p>
            </div>
          ) : (
        ...
        <div className="glass p-6">
          ...
          {recentLeaves.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <p className="text-[var(--text-muted)] font-medium text-[13px]">No leave requests yet</p>
            </div>
          ) : (
            <div className="space-y-2">
              {recentLeaves.map((leave) => (
                <div key={leave._id as string}
                  className="flex items-center justify-between p-3.5 bg-[rgba(255,255,255,0.02)] rounded-xl border border-[var(--glass-border)] hover:bg-[rgba(255,255,255,0.04)] transition-all duration-200">
                  <div>
                    ...
                    <p className="text-[11px] text-[var(--text-muted)]">{(leave.userId as Record<string, unknown>)?.department as string}</p>
                  </div>
                  <span className={`px-2.5 py-1 text-[10px] font-semibold rounded-lg
                    ${leave.status === 'pending' ? 'bg-[rgba(251,191,36,0.12)] text-[var(--color-warning)]' :
                      leave.status === 'approved' ? 'bg-[rgba(52,211,153,0.12)] text-[var(--color-success)]' :
